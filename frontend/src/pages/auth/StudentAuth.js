...
import {Studentregister,Studentlogin}from "../../utils/api"
...
const StudentAuth = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = await Studentregister(email, password);
  ...
  const handleSigninSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = await Studentlogin(Signinemail, Signinpassword);
      setMessage(data.message);
  ...
  return (
    <div>
        ...
        <div className="login-btn">
            ...
            >
                <StudentIcon fill='#fff'/>
                <p className={isActive('/') ? 'active' : ''}>STUDENT</p>
            </a>
            <a
                href="/TrainerAuth"
