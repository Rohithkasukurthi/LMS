...
const StudentModel = require('../models/StudentModel'); // Correct path for the User model
...
StudentRoutes.post('/login', async (req, res) => {
  try {
    ...
    const admin = await StudentModel.findOne({ email }).select('+password'); // Ensure the password field is selected
    if (!admin) {
...
StudentRoutes.get('/dashboard', authMiddleware, async (req, res) => {
  try {
