...
const TpoRoutes = express.Router();
const cookieParser = require('cookie-parser');
const TpoModel = require('../models/TpoModel'); // Correct path for the User model
...
TpoRoutes.post('/register', async (req, res) => {
  try {
    ...
    const existingUser = await TpoModel.findOne({ email });
    if (existingUser) {
    ...
    // Create the user
    const newUser = await TpoModel.create({
      email,
...
// Login route
TpoRoutes.post('/login', async (req, res) => {
  try {
    const { email, password } = req.body;
    ...
    const admin = await TpoModel.findOne({ email }).select('+password'); // Ensure the password field is selected
    if (!admin) {
...
TpoRoutes.get('/dashboard', authMiddleware, async (req, res) => {
  try {
